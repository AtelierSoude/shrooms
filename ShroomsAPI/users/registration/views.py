"""
Forked from the Django Registration Project
...
from users.registration import signals, serializers
...
class RegistrationView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        """
        Custom create method that allow registering new users.
        Dispatches signals and triggers sending activation email.
        """
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        new_user = self.perform_create(serializer)
        #action.send(request.user, verb='a inscrit', target=new_user)
...
class ActivationView(views.APIView):
    ...
    def get(self, request, activation_key):
        """
        The base activation logic.
        """
        activated_user = self.activate(activation_key)
        if activated_user:
            ...
            # TODO :
            # add activity stream signal here
            return Response(
                serializers.UserActivatedSerializer(activated_user).data,
                status=status.HTTP_201_CREATED)

        else:
            return Response(
                {'message': _('Invalid activation key')},
                status=status.HTTP_400_BAD_REQUEST)

    def activate(self, activation_key):
        """
        Attempts to match the activation key with an existing user.
        Returns user instance if matching occurs.
        """
